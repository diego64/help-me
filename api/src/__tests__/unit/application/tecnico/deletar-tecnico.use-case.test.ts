import { describe, it, expect, vi, beforeEach } from 'vitest'
import type { Regra } from '@prisma/client'

import { deletarTecnicoUseCase } from '@application/use-cases/tecnico/deletar-tecnico.use-case'
import { TecnicoError } from '@application/use-cases/tecnico/errors'
import { prisma } from '@infrastructure/database/prisma/client'
import { logger } from '@shared/config/logger'

const mockExpedienteDeleteMany = vi.fn()
const mockUsuarioDelete = vi.fn()

vi.mock('@infrastructure/database/prisma/client', () => ({
  prisma: {
    usuario: {
      findUnique: vi.fn(),
      update: vi.fn(),
      delete: vi.fn(),
    },
    $transaction: vi.fn(),
  },
}))

vi.mock('@shared/config/logger', () => ({
  logger: {
    info: vi.fn(),
    error: vi.fn(),
    warn: vi.fn(),
    debug: vi.fn(),
  },
}))

const DATA_FIXA = new Date('2024-01-01T00:00:00.000Z')

const makeInput = (overrides = {}): Parameters<typeof deletarTecnicoUseCase>[0] => ({
  id: 'tecnico-id-123',
  permanente: false,
  ...overrides,
})

const makeTecnico = (overrides = {}) => ({
  id: 'tecnico-id-123',
  regra: 'TECNICO' as Regra,
  email: 'joao@email.com',
  deletadoEm: null,
  geradoEm: DATA_FIXA,
  atualizadoEm: DATA_FIXA,
  _count: { tecnicoChamados: 0 },
  ...overrides,
})

beforeEach(() => {
  vi.clearAllMocks()

  mockExpedienteDeleteMany.mockResolvedValue({ count: 0 })
  mockUsuarioDelete.mockResolvedValue(makeTecnico())

  vi.mocked(prisma.usuario.findUnique).mockResolvedValue(makeTecnico() as any)
  vi.mocked(prisma.usuario.update).mockResolvedValue(makeTecnico({ ativo: false, deletadoEm: new Date() }) as any)
  vi.mocked(prisma.$transaction).mockImplementation((async (fn: any) => fn({
    expediente: { deleteMany: mockExpedienteDeleteMany },
    usuario: { delete: mockUsuarioDelete },
  })) as any)
})

describe('deletarTecnicoUseCase', () => {
  describe('verificação de existência do técnico', () => {
    it('deve buscar técnico pelo id', async () => {
      await deletarTecnicoUseCase(makeInput())

      expect(prisma.usuario.findUnique).toHaveBeenCalledWith(
        expect.objectContaining({ where: { id: 'tecnico-id-123' } })
      )
    })

    it('deve lançar TecnicoError quando técnico não existir', async () => {
      vi.mocked(prisma.usuario.findUnique).mockResolvedValue(null)

      await expect(deletarTecnicoUseCase(makeInput())).rejects.toThrow(TecnicoError)
    })

    it('deve lançar TecnicoError com mensagem correta quando não encontrado', async () => {
      vi.mocked(prisma.usuario.findUnique).mockResolvedValue(null)

      await expect(deletarTecnicoUseCase(makeInput())).rejects.toThrow('Técnico não encontrado')
    })

    it('deve lançar TecnicoError com code NOT_FOUND quando não encontrado', async () => {
      vi.mocked(prisma.usuario.findUnique).mockResolvedValue(null)

      const error = await deletarTecnicoUseCase(makeInput()).catch(e => e)
      expect(error.code).toBe('NOT_FOUND')
    })

    it('deve lançar TecnicoError com statusCode 404 quando não encontrado', async () => {
      vi.mocked(prisma.usuario.findUnique).mockResolvedValue(null)

      const error = await deletarTecnicoUseCase(makeInput()).catch(e => e)
      expect(error.statusCode).toBe(404)
    })

    it('deve lançar TecnicoError quando usuário existe mas não é TECNICO', async () => {
      vi.mocked(prisma.usuario.findUnique).mockResolvedValue(
        makeTecnico({ regra: 'USUARIO' as Regra }) as any
      )

      await expect(deletarTecnicoUseCase(makeInput())).rejects.toThrow(TecnicoError)
    })
  })

  describe('soft delete (padrão)', () => {
    it('deve executar soft delete quando permanente=false', async () => {
      await deletarTecnicoUseCase(makeInput({ permanente: false }))

      expect(prisma.usuario.update).toHaveBeenCalledWith({
        where: { id: 'tecnico-id-123' },
        data: {
          deletadoEm: expect.any(Date),
          ativo: false,
        },
      })
    })

    it('não deve chamar transaction no soft delete', async () => {
      await deletarTecnicoUseCase(makeInput({ permanente: false }))

      expect(prisma.$transaction).not.toHaveBeenCalled()
    })

    it('deve retornar mensagem e id corretos no soft delete', async () => {
      const result = await deletarTecnicoUseCase(makeInput({ permanente: false }))

      expect(result).toEqual({
        message: 'Técnico deletado com sucesso',
        id: 'tecnico-id-123',
      })
    })

    it('deve logar info após soft delete', async () => {
      await deletarTecnicoUseCase(makeInput({ permanente: false }))

      expect(logger.info).toHaveBeenCalledWith(
        { tecnicoId: 'tecnico-id-123', email: 'joao@email.com' },
        '[TECNICO] Soft delete realizado'
      )
    })
  })

  describe('hard delete (permanente)', () => {
    it('deve lançar TecnicoError com code HAS_CHAMADOS quando há chamados vinculados', async () => {
      vi.mocked(prisma.usuario.findUnique).mockResolvedValue(
        makeTecnico({ _count: { tecnicoChamados: 2 } }) as any
      )

      const error = await deletarTecnicoUseCase(makeInput({ permanente: true })).catch(e => e)

      expect(error).toBeInstanceOf(TecnicoError)
      expect(error.code).toBe('HAS_CHAMADOS')
    })

    it('deve lançar TecnicoError com statusCode 400 quando há chamados vinculados', async () => {
      vi.mocked(prisma.usuario.findUnique).mockResolvedValue(
        makeTecnico({ _count: { tecnicoChamados: 1 } }) as any
      )

      const error = await deletarTecnicoUseCase(makeInput({ permanente: true })).catch(e => e)
      expect(error.statusCode).toBe(400)
    })

    it('deve executar hard delete via transaction quando permanente=true e sem chamados', async () => {
      await deletarTecnicoUseCase(makeInput({ permanente: true }))

      expect(prisma.$transaction).toHaveBeenCalledTimes(1)
    })

    it('deve deletar expedientes dentro da transaction', async () => {
      await deletarTecnicoUseCase(makeInput({ permanente: true }))

      expect(mockExpedienteDeleteMany).toHaveBeenCalledWith({
        where: { usuarioId: 'tecnico-id-123' },
      })
    })

    it('deve deletar o técnico dentro da transaction', async () => {
      await deletarTecnicoUseCase(makeInput({ permanente: true }))

      expect(mockUsuarioDelete).toHaveBeenCalledWith({
        where: { id: 'tecnico-id-123' },
      })
    })

    it('não deve chamar update no hard delete', async () => {
      await deletarTecnicoUseCase(makeInput({ permanente: true }))

      expect(prisma.usuario.update).not.toHaveBeenCalled()
    })

    it('deve retornar mensagem de exclusão permanente', async () => {
      const result = await deletarTecnicoUseCase(makeInput({ permanente: true }))

      expect(result).toEqual({
        message: 'Técnico removido permanentemente',
        id: 'tecnico-id-123',
      })
    })

    it('deve logar info após hard delete', async () => {
      await deletarTecnicoUseCase(makeInput({ permanente: true }))

      expect(logger.info).toHaveBeenCalledWith(
        { tecnicoId: 'tecnico-id-123', email: 'joao@email.com' },
        '[TECNICO] Excluído permanentemente'
      )
    })
  })

  describe('tratamento de erros', () => {
    it('deve relançar TecnicoError sem encapsular', async () => {
      vi.mocked(prisma.usuario.findUnique).mockResolvedValue(null)

      const error = await deletarTecnicoUseCase(makeInput()).catch(e => e)

      expect(error).toBeInstanceOf(TecnicoError)
      expect(error.code).toBe('NOT_FOUND')
    })

    it('deve lançar TecnicoError com code DELETE_ERROR quando update falhar', async () => {
      vi.mocked(prisma.usuario.update).mockRejectedValue(new Error('Database error'))

      const error = await deletarTecnicoUseCase(makeInput()).catch(e => e)

      expect(error).toBeInstanceOf(TecnicoError)
      expect(error.code).toBe('DELETE_ERROR')
    })

    it('deve lançar TecnicoError com code DELETE_ERROR quando transaction falhar', async () => {
      vi.mocked(prisma.$transaction).mockRejectedValue(new Error('Transaction error'))

      const error = await deletarTecnicoUseCase(makeInput({ permanente: true })).catch(e => e)

      expect(error).toBeInstanceOf(TecnicoError)
      expect(error.code).toBe('DELETE_ERROR')
    })

    it('deve lançar TecnicoError com statusCode 500 quando operação falhar', async () => {
      vi.mocked(prisma.usuario.update).mockRejectedValue(new Error('Database error'))

      const error = await deletarTecnicoUseCase(makeInput()).catch(e => e)
      expect(error.statusCode).toBe(500)
    })

    it('deve lançar TecnicoError com mensagem correta quando operação falhar', async () => {
      vi.mocked(prisma.usuario.update).mockRejectedValue(new Error('Database error'))

      await expect(deletarTecnicoUseCase(makeInput())).rejects.toThrow('Erro ao deletar técnico')
    })

    it('deve incluir originalError quando falha com instância de Error', async () => {
      const dbError = new Error('Database error')
      vi.mocked(prisma.usuario.update).mockRejectedValue(dbError)

      const error = await deletarTecnicoUseCase(makeInput()).catch(e => e)
      expect(error.originalError).toBe(dbError)
    })

    it('deve logar erro quando operação falhar', async () => {
      const dbError = new Error('Database error')
      vi.mocked(prisma.usuario.update).mockRejectedValue(dbError)

      await deletarTecnicoUseCase(makeInput()).catch(() => {})

      expect(logger.error).toHaveBeenCalledWith(
        { error: dbError, tecnicoId: 'tecnico-id-123' },
        '[TECNICO] Erro ao deletar'
      )
    })
  })
})
