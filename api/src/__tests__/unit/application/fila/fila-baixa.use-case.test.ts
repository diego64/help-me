...
const makeInput = (overrides = {}): Parameters<typeof filaBaixaUseCase>[0] => ({
  page: 1,
  limit: 10,
...
describe('filaBaixaUseCase', () => {
  describe('controle de acesso por nível de técnico', () => {
    it('deve permitir acesso para ADMIN', async () => {
      await expect(filaBaixaUseCase(makeInput({ usuarioRegra: 'ADMIN' }))).resolves.toBeDefined()
    })

    it('deve permitir acesso para técnico N1', async () => {
      vi.mocked(prisma.usuario.findUnique).mockResolvedValue(makeTecnico({ nivel: NivelTecnico.N1 }) as any)

      await expect(
        filaBaixaUseCase(makeInput({ usuarioRegra: 'TECNICO' }))
      ).resolves.toBeDefined()
    })

    it('deve bloquear acesso para técnico N2', async () => {
    ...
    it('deve bloquear acesso para técnico N3', async () => {
    ...
    it('deve lançar FilaError com code FORBIDDEN para técnico N2', async () => {
      vi.mocked(prisma.usuario.findUnique).mockResolvedValue(makeTecnico({ nivel: NivelTecnico.N2 }) as any)

      const error = await filaBaixaUseCase(makeInput({ usuarioRegra: 'TECNICO' })).catch(e => e)

      expect(error).toBeInstanceOf(FilaError)
      expect(error.code).toBe('FORBIDDEN')
    })

    it('deve lançar FilaError com statusCode 403 para técnico N2', async () => {
      vi.mocked(prisma.usuario.findUnique).mockResolvedValue(makeTecnico({ nivel: NivelTecnico.N2 }) as any)
    ...
    it('deve lançar erro com mensagem específica para técnico N2', async () => {
      vi.mocked(prisma.usuario.findUnique).mockResolvedValue(makeTecnico({ nivel: NivelTecnico.N2 }) as any)

      await expect(
        filaBaixaUseCase(makeInput({ usuarioRegra: 'TECNICO' }))
      ).rejects.toThrow('Técnicos N2 não têm acesso à fila de baixa prioridade')
    })

    it('deve lançar erro com mensagem específica para técnico N3', async () => {
      vi.mocked(prisma.usuario.findUnique).mockResolvedValue(makeTecnico({ nivel: NivelTecnico.N3 }) as any)

      await expect(
        filaBaixaUseCase(makeInput({ usuarioRegra: 'TECNICO' }))
      ).rejects.toThrow('Técnicos N3 não têm acesso à fila de baixa prioridade')
    })

    it('não deve buscar nível do usuário quando regra é ADMIN', async () => {
      await filaBaixaUseCase(makeInput({ usuarioRegra: 'ADMIN' }))

      expect(prisma.usuario.findUnique).not.toHaveBeenCalled()
    })

    it('deve buscar nível do usuário quando regra é TECNICO', async () => {
      await filaBaixaUseCase(makeInput({ usuarioRegra: 'TECNICO', usuarioId: 'tecnico-id-456' }))
  ...
  describe('filtros da query', () => {
    it('deve filtrar por status ABERTO e REABERTO', async () => {
      await filaBaixaUseCase(makeInput())
    ...
    it('deve filtrar por prioridade P4 e P5', async () => {
      await filaBaixaUseCase(makeInput())
  ...
  describe('paginação', () => {
    it('deve calcular skip corretamente para page=1', async () => {
      await filaBaixaUseCase(makeInput({ page: 1, limit: 10 }))
    ...
    it('deve calcular skip corretamente para page=2', async () => {
      await filaBaixaUseCase(makeInput({ page: 2, limit: 10 }))
    ...
    it('deve calcular skip corretamente para page=3 com limit=5', async () => {
      await filaBaixaUseCase(makeInput({ page: 3, limit: 5 }))
    ...
    it('deve usar take igual ao limit', async () => {
      await filaBaixaUseCase(makeInput({ limit: 25 }))
  ...
  describe('ordenação customizada', () => {
    it('deve reordenar chamados por prioridade e depois por data', async () => {
      const chamados = [
        makeChamado({ id: 'c1', prioridade: PrioridadeChamado.P5, geradoEm: DATA_FIXA_1 }),
        makeChamado({ id: 'c2', prioridade: PrioridadeChamado.P4, geradoEm: DATA_FIXA_2 }),
        makeChamado({ id: 'c3', prioridade: PrioridadeChamado.P4, geradoEm: DATA_FIXA_1 }),
      ]
      vi.mocked(prisma.chamado.findMany).mockResolvedValue(chamados as any)

      const result = await filaBaixaUseCase(makeInput())

      // P4 (mais antiga) -> P4 (mais recente) -> P5
    ...
    it('deve ordenar por prioridade quando datas iguais', async () => {
      const chamados = [
        makeChamado({ id: 'c1', prioridade: PrioridadeChamado.P5, geradoEm: DATA_FIXA_1 }),
        makeChamado({ id: 'c2', prioridade: PrioridadeChamado.P4, geradoEm: DATA_FIXA_1 }),
      ]
      vi.mocked(prisma.chamado.findMany).mockResolvedValue(chamados as any)

      const result = await filaBaixaUseCase(makeInput())

      expect(result.data[0].id).toBe('c2') // P4 vem antes de P5
      expect(result.data[1].id).toBe('c1')
    })

    it('deve ordenar por data quando mesma prioridade', async () => {
      const chamados = [
        makeChamado({ id: 'c1', prioridade: PrioridadeChamado.P4, geradoEm: DATA_FIXA_2 }),
        makeChamado({ id: 'c2', prioridade: PrioridadeChamado.P4, geradoEm: DATA_FIXA_1 }),
      ]
      vi.mocked(prisma.chamado.findMany).mockResolvedValue(chamados as any)
  ...
  describe('retorno da resposta', () => {
    it('deve retornar fila="BAIXA"', async () => {
      const result = await filaBaixaUseCase(makeInput())

      expect(result.fila).toBe('BAIXA')
    })

    it('deve retornar prioridades [P4, P5]', async () => {
      const result = await filaBaixaUseCase(makeInput())

      expect(result.prioridades).toEqual(['P4', 'P5'])
    })
    ...
    it('deve retornar pagination com page correto', async () => {
      const result = await filaBaixaUseCase(makeInput({ page: 3 }))

      expect(result.pagination.page).toBe(3)
    })

    it('deve retornar pagination com limit correto', async () => {
      const result = await filaBaixaUseCase(makeInput({ limit: 25 }))
    ...
    it('deve calcular totalPages corretamente', async () => {
      ...
      const result = await filaBaixaUseCase(makeInput({ limit: 10 }))
    ...
    it('deve retornar hasNext=true quando há próxima página', async () => {
      ...
      const result = await filaBaixaUseCase(makeInput({ page: 1, limit: 10 }))
    ...
    it('deve retornar hasNext=false quando é a última página', async () => {
      ...
      const result = await filaBaixaUseCase(makeInput({ page: 3, limit: 10 }))

      expect(result.pagination.hasNext).toBe(false)
    })

    it('deve retornar hasPrev=false quando page=1', async () => {
      const result = await filaBaixaUseCase(makeInput({ page: 1 }))

      expect(result.pagination.hasPrev).toBe(false)
    })

    it('deve retornar hasPrev=true quando page>1', async () => {
      const result = await filaBaixaUseCase(makeInput({ page: 2 }))
  ...
  describe('logging', () => {
    it('deve logar sucesso com page, limit e total', async () => {
      ...
      await filaBaixaUseCase(makeInput({ page: 2, limit: 5 }))

      expect(logger.info).toHaveBeenCalledWith(
        { page: 2, limit: 5, total: 15 },
        '[FILA] Fila baixa consultada'
      )
    })
  ...
  describe('tratamento de erros', () => {
    it('deve relançar FilaError sem encapsular quando já é FilaError', async () => {
      ...
      const error = await filaBaixaUseCase(makeInput()).catch(e => e)
    ...
    it('deve lançar FilaError com code FILA_BAIXA_ERROR quando count falhar', async () => {
      vi.mocked(prisma.chamado.count).mockRejectedValue(new Error('Database error'))

      const error = await filaBaixaUseCase(makeInput()).catch(e => e)

      expect(error).toBeInstanceOf(FilaError)
      expect(error.code).toBe('FILA_BAIXA_ERROR')
    })

    it('deve lançar FilaError com code FILA_BAIXA_ERROR quando findMany falhar', async () => {
    ...
    it('deve lançar FilaError com mensagem correta quando operação falhar', async () => {
      ...
      await expect(filaBaixaUseCase(makeInput())).rejects.toThrow(
        'Erro ao buscar fila de baixa prioridade'
      )
    })
    ...
    it('deve logar erro quando operação falhar', async () => {
      ...
      await filaBaixaUseCase(makeInput()).catch(() => {})

      expect(logger.error).toHaveBeenCalledWith(
        { error: dbError },
        '[FILA] Erro ao buscar fila baixa'
      )
    })
