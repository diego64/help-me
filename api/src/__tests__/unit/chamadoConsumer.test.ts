...
import { startChamadoConsumer, stopChamadoConsumer, isChamadoConsumerRunning, resetConsumerState, renderTemplate, sendChamadoAbertoEmail, sendChamadoEncerradoEmail, processKafkaMessage, ConsumerError, TemplateError, EmailError, ChamadoData } from '../../infrastructure/messaging/kafka/consumers/chamadoConsumer';
...
// Helper para criar mock de EachMessagePayload
...
describe('ChamadoConsumer', () => {
  ...
  describe('renderTemplate', () => {
    ...
    it('deve lançar erro quando templatePath não for string', () => {
      ...
      expect(() => renderTemplate(undefined as any, mockData)).toThrow(
        TemplateError
      );
    });
  ...
  describe('sendChamadoAbertoEmail', () => {
    ...
    it('deve lançar erro quando dados do chamado forem undefined', async () => {
      await expect(sendChamadoAbertoEmail(undefined as any)).rejects.toThrow(
        ConsumerError
      );
    });
    ...
    it('deve lançar erro quando múltiplos campos estiverem ausentes', async () => {
      ...
      const erro = await sendChamadoAbertoEmail(chamadoInvalido as any).catch(
        (e) => e
      );
    ...
    it('deve propagar TemplateError quando renderTemplate falhar', async () => {
      ...
      await expect(sendChamadoAbertoEmail(chamadoValido)).rejects.toThrow(
        TemplateError
      );
    });
  ...
  describe('sendChamadoEncerradoEmail', () => {
    ...
    it('deve lançar erro quando dados do chamado forem inválidos', async () => {
      await expect(sendChamadoEncerradoEmail(null as any)).rejects.toThrow(
        ConsumerError
      );
    });
  ...
  describe('processKafkaMessage', () => {
    ...
    it('não deve propagar erro para não parar o consumer', async () => {
      ...
      await expect(processKafkaMessage(createMockPayload(chamado))).resolves.not.toThrow();
    });
  });
  ...
  describe('startChamadoConsumer', () => {
    ...
    it('deve reutilizar consumer instance existente', async () => {
      ...
      // Simula que o consumer ainda existe mas não está rodando
      await startChamadoConsumer();

      // kafka.consumer deve ser chamado apenas uma vez na segunda execução
