 ...
 * /api/fila-chamados/todos-chamados:
 *   get:
 *     summary: Lista chamados filtrados por status
 *     description: Retorna todos os chamados filtrados pelo status informado. Requer autenticação e perfil ADMIN ou TECNICO.
 *     tags: [Listagens]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *         schema:
 *           type: string
 *           enum: [ABERTO, EM_ANDAMENTO, RESOLVIDO, FECHADO, CANCELADO, REABERTO]
 ...
 *         description: Lista de chamados retornada com sucesso
 *       400:
 *         description: Status inválido ou não informado
 *       401:
 *         description: Não autenticado
 *       403:
 *         description: Sem permissão
...
router.get(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (!statusValidos.includes(status as ChamadoStatus)) {
        return res.status(400).json({
          error: 'Status inválido',
          statusValidos,
        });
      ...
      console.error('[LISTAGEM POR STATUS ERROR]', err);
      res.status(500).json({
        error: 'Erro ao listar chamados por status',
      });
    }
  }
);
